...
local function EditItem(source, item, slot, key, value)
	local user = QBCore.Functions.GetPlayer(source)
	if not user then return false end
	local AllItems = user.PlayerData.items
	if slot ~= nil then
		AllItems[slot].info[key] = value
		user.Functions.SetPlayerData("items", AllItems)
		return true
	else
		slot = GetFirstSlotByItem(AllItems, item)
		AllItems[slot].info[key] = value
		user.Functions.SetPlayerData("items", AllItems)
		return true
	end
...
local function GetStashItems(stashId)
	...
	for slot, item in pairs(stashItems) do
		...
		if item.type ~= 'weapon' then
			local quality = ConvertQuality(item.name, item.info.date)
			newData[slot].info.quality = quality
		end
	end
	stashItems = newData
...
local function OpenInventory(name, id, other, origin)
	...
	if name and id then
		...
		Player.Functions.SetPlayerData("items", newData)
		Wait(10)
	...
	if name and id then
		...
		elseif name == "otherplayer" then
			...
			if Drops[id] and not Drops[id].isOpen then
				secondInv.coords = Drops[id].coords
				secondInv.name = id
				secondInv.label = "Yer-"..tostring(id)
				secondInv.maxweight = 100000
				secondInv.inventory = Drops[id].items
				secondInv.slots = 30
				Drops[id].isOpen = src
...
RegisterNetEvent('inventory:server:UseItemSlot', function(slot)
	...
	if itemData.info.quality <= 1 then
		QBCore.Functions.Notify(src, 'Item is too worn.', 'error', 2500)
		return
	end
...
QBCore.Commands.Add("giveitem", "Give Item", {{name="id", help="Player ID"},{name="item", help="Name of the item (not a label)"}, {name="amount", help="Amount of items"}}, false, function(source, args)
